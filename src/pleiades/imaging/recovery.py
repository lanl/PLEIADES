...
class PhysicsRecovery:
    ...
    def _run_sammy_forward_model(
        ...
        if working_dir is None:
            tmp = tempfile.mkdtemp(prefix=f"recovery_{isotope_name}_")
            working_dir = Path(tmp)
            cleanup_dir = True

        try:
            pixel_dir = working_dir / f"ref_{isotope_name}"
    ...
    def recover_pixel(
        ...
        """Recover isotopic areal densities for a single pixel via NNLS.

        Solves ``min_n ||W @ y - W @ D @ n||^2  s.t.  n >= 0`` where:
        - ``y = -ln(T_obs)`` (observed absorption)
        - ``D`` is the dictionary matrix of reference absorptions
        ...
        Returns:
            ...
            - ``coefficients``: Recovered areal density coefficients, shape ``(n_isotopes,)``.
            - ``chi_squared``: Chi-squared value of the fit.
            - ``success``: Whether NNLS converged (always True for scipy.optimize.nnls).
        """
