...
# def get_proton_charge_dict(list_sample_nexus: list,
#                             list_nexus_obs: list,
#                             nbr_sample_folders: int,
#                             nbr_obs_folders: int) -> dict:
#     """
...
#     """
#     logger.info(f"Checking if normalization by proton charge is possible")

#     proton_charge_dict = {'state': False,
#                           'sample': None,
#                           'ob': None}

#     if len(list_sample_nexus) != nbr_sample_folders:
#        logger.info(f"Number of sample nexus files ({len(list_sample_nexus)}) does not match the number of sample folders ({nbr_sample_folders})")
#        return proton_charge_dict
    
#     if len(list_nexus_obs) != nbr_obs_folders:
#        logger.info(f"Number of open beam nexus files ({len(list_nexus_obs)}) does not match the number of open beam folders ({nbr_obs_folders})")
#        return proton_charge_dict
    
#     sample_list_proton_charge = [get_proton_charge(nexus) for nexus in list_sample_nexus]
#     if None in sample_list_proton_charge:
#         logger.info(f"Some of the proton charge are missing from the sample nexus files")
#         return proton_charge_dict
    
#     ob_list_proton_charge = [get_proton_charge(nexus) for nexus in list_nexus_obs]
#     if None in ob_list_proton_charge:
#         logger.info(f"Some of the proton charge are missing from the open beam nexus files")
#         return proton_charge_dict
    
#     # If all checks pass, normalization by proton charge is possible

#     proton_charge_dict = {'state': True,
#                           'sample': sample_list_proton_charge,
#                           'ob': ob_list_proton_charge}

#     logger.info(f"Normalization by proton charge is possible")
