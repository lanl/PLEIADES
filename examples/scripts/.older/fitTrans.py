...
def main(config_inp_file: str = "config_inp.ini"):
    ...
    sammy_input.data["Card1"]["title"] = (
        "Run SAMMY to find abundance of uranium-235 isotope"  # modifying Title
    )
    ...
    sammy_input.data["Card3"]["commands"] = (
        "CHI_SQUARED,TWENTY,SOLVE_BAYES,QUANTUM_NUMBERS,GENERATE ODF FILE AUTOMATICALLY,REICH_MOORE_FORM"
    )
    ...
    psp.process_and_plot_lst_file(
        "archive/U_235/results/U_235.LST", residual=True, quantity="transmission"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process config file for sammy input, run sammy, and plot results."
    )
    parser.add_argument(
        "--sammyConfig",
        type=str,
        default="config.ini",
        help="Path to the sammy config file",
    )
