...
# ===========================================================================
# TestCreateSammyRunner: tests for _create_sammy_runner() helper function
# ===========================================================================


class TestCreateSammyRunner:
    """Tests for the _create_sammy_runner() factory function.

    This function creates the appropriate SammyRunner subclass based on the
    backend string. It is called from _fit_pixel_worker to construct the runner.
    """

    def test_create_local_runner(self, tmp_path):
        """_create_sammy_runner('local') returns a LocalSammyRunner instance."""
        from pleiades.imaging.orchestrator import _create_sammy_runner
        ...
        runner = _create_sammy_runner(
            ...
            sammy_executable=sammy_exe,
        )

        assert isinstance(runner, LocalSammyRunner)

    def test_create_docker_runner(self, tmp_path):
        """_create_sammy_runner('docker') returns a DockerSammyRunner instance."""
        from pleiades.imaging.orchestrator import _create_sammy_runner
        ...
        runner = _create_sammy_runner(
            ...
            working_dir=working_dir,
            output_dir=output_dir,
            backend_kwargs={"image_name": "kedokudo/sammy-docker"},
        )

        assert isinstance(runner, DockerSammyRunner)

    def test_create_unknown_backend_raises(self, tmp_path):
        """_create_sammy_runner raises ValueError for unrecognized backend strings."""
        from pleiades.imaging.orchestrator import _create_sammy_runner
    ...
    def test_create_local_runner_uses_sammy_executable(self, tmp_path):
        """Local runner's config receives the sammy_executable path."""
        from pleiades.imaging.orchestrator import _create_sammy_runner
        ...
        assert runner.config.sammy_executable == sammy_exe

    def test_create_docker_runner_uses_backend_kwargs(self, tmp_path):
        """Docker runner's config receives image_name from backend_kwargs."""
        from pleiades.imaging.orchestrator import _create_sammy_runner
        ...
        runner = _create_sammy_runner(
            ...
            working_dir=working_dir,
            output_dir=output_dir,
            backend_kwargs={"image_name": "my-custom-sammy:v2"},
        )

        assert runner.config.image_name == "my-custom-sammy:v2"

    def test_create_local_runner_default_executable(self, tmp_path):
        """When no sammy_executable is given for local, use a sensible default or raise."""
        from pleiades.imaging.orchestrator import _create_sammy_runner

        working_dir = tmp_path / "work"
        output_dir = tmp_path / "output"

        # Without sammy_executable for local backend, the function should either:
        # a) raise an error (sammy_executable is required for local), OR
        # b) use a default from backend_kwargs
        # We test that passing it via backend_kwargs works as a fallback.
        sammy_exe = tmp_path / "sammy"
        sammy_exe.touch()
        sammy_exe.chmod(0o755)
        ...
        runner = _create_sammy_runner(
            ...
            working_dir=working_dir,
            output_dir=output_dir,
            backend_kwargs={"sammy_executable": sammy_exe},
        )
...
# ===========================================================================
# TestBackendAbstraction: tests for backend= parameter on orchestrator
# ===========================================================================


class TestBackendAbstraction:
    """Tests for BatchFittingOrchestrator backend abstraction.

    The orchestrator's __init__ gains backend= and backend_kwargs= parameters.
    The worker function gains backend= and backend_kwargs= parameters.
    """

    def test_init_default_backend_is_local(self, imaging_config, mock_sammy_executable):
        """Default backend should be 'local' for backward compatibility."""
    ...
    def test_init_local_backend_requires_executable(self, imaging_config):
        """Local backend without sammy_executable raises FileNotFoundError or ValueError."""
        # When backend='local' and no sammy_executable is provided at all,
        # the orchestrator should raise because it can't run SAMMY locally.
    ...
    def test_init_docker_backend_no_executable_needed(self, imaging_config):
        """Docker backend works without sammy_executable."""
        orchestrator = BatchFittingOrchestrator(
            imaging_config=imaging_config,
            n_workers=4,
            backend="docker",
    ...
    def test_init_invalid_backend_raises(self, imaging_config, mock_sammy_executable):
    ...
    def test_init_local_with_backend_kwargs_executable(self, imaging_config, tmp_path):
        """sammy_executable can be provided via backend_kwargs for local backend."""
        sammy_exe = tmp_path / "sammy"
        sammy_exe.touch()
        sammy_exe.chmod(0o755)
        ...
        assert orchestrator.sammy_executable == sammy_exe

    def test_init_preserves_backward_compat(self, imaging_config, mock_sammy_executable):
        """Old-style API (positional sammy_executable, no backend=) still works."""
        orchestrator = BatchFittingOrchestrator(
            imaging_config=imaging_config,
            sammy_executable=mock_sammy_executable,
            n_workers=4,
    ...
    def test_worker_receives_backend_params(
        self, mock_executor_cls, mock_json_mgr, imaging_config, mock_sammy_executable
    ):
        """fit_pixels passes backend and backend_kwargs to each submitted worker call."""
        ...
        def submit_side_effect(fn, *args, **kwargs):
            submitted_kwargs.append({"args": args, "kwargs": kwargs})
            pixel = args[0]
            future = Future()
        ...
        orchestrator = BatchFittingOrchestrator(
            ...
            n_workers=2,
            backend="local",
            backend_kwargs={"some_option": True},
        )
        results = orchestrator.fit_pixels(pixels)

        assert len(results) == 2
        # Each submit call should have received backend and backend_kwargs as positional args.
        # Worker signature: (pixel, imaging_config, sammy_exe, resolution_file,
        #                    shared_json, shared_endf, backend, backend_kwargs)
        # So args[6] = backend, args[7] = backend_kwargs
        for call_info in submitted_kwargs:
            args = call_info["args"]
            assert args[6] == "local", f"Expected backend='local' at position 6, got: {args[6]}"
            assert args[7] == {"some_option": True}, f"Expected backend_kwargs at position 7, got: {args[7]}"

    @patch("pleiades.imaging.orchestrator._create_sammy_runner")
    @patch("pleiades.imaging.orchestrator.ResultsManager")
    @patch("pleiades.imaging.orchestrator.JsonManager")
    def test_docker_backend_worker_uses_docker_runner(
        self, mock_json_mgr, mock_results_mgr_cls, mock_create_runner, imaging_config
    ):
        """Worker creates DockerSammyRunner when backend='docker'."""
        ...
        # Mock _create_sammy_runner to return a mock runner
        ...
        mock_chi_sq = MagicMock()
        mock_chi_sq.chi_squared = 1.5
        ...
        result = _fit_pixel_worker(
            pixel,
            imaging_config,
            sammy_executable=None,
            backend="docker",
            backend_kwargs={"image_name": "kedokudo/sammy-docker"},
        )

        assert result.success is True
        # Verify _create_sammy_runner was called with backend="docker"
        mock_create_runner.assert_called_once()
        call_kwargs = mock_create_runner.call_args.kwargs
        assert call_kwargs["backend"] == "docker", (
            f"Expected _create_sammy_runner to be called with backend='docker', got: {call_kwargs.get('backend')}"
        )
