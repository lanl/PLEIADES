...
def single_run(
    ...
    """
    Run the SAMMY program for a single set of input files.

    Args:
        fit_dir (str): The directory where the input files are located.
        input_file (str): The name of the input file.
        par_file (str): The name of the parameter file.
        data_file (str): The name of the data file.
        output_dir (str, optional): The directory where the output files will be saved. Defaults to "results".
        verbose_level (int, optional): The level of verbosity for printing information. Defaults to 0.

    Raises:
        ValueError: If any of the required input files are missing.
    ...
    if verbose_level > 0:
        print(f"Running SAMMY for {fit_dir}")
    ...
    sammy_run_command = textwrap.dedent(
        f"""\
    sammy <<EOF
    {input_file}
    {par_file}
...
def run_endf(
    ...
    Args:
        run_handle (str): Run or handle name. This is what will be used for the DAT and PAR 
        file names.
    ...
    if verbose_level > 0:
        print(f"Working directory: {endf_path}")
    ...
    with open(output_file_name, "w") as output:
        # Run the command and redirect output and error to the file
        if verbose_level > 0:
            print(f"Running SAMMY for {run_handle}...")
        subprocess.run(
            sammy_run_command,
            shell=True,
            executable="/bin/bash",
...
def run(
    ...
    Args:
        archivename (str): Archive directory name. If only archivename is provided, the other 
        file names will be assumed to have the same name as the archive with the associated 
        extension, e.g., {archivename}.inp.
    ...
    try:
        shutil.move(
            archivepath / "SAMMY.LST", archivepath / f"results/{archivename}.lst"
        )
    except FileNotFoundError:
        print("lst file is not found")
    try:
        shutil.move(
            archivepath / "SAMMY.LPT", archivepath / f"results/{archivename}.lpt"
        )
    except FileNotFoundError:
    ...
    try:
        shutil.move(
            archivepath / "SAMMY.PAR", archivepath / f"results/{archivename}.par"
        )
        # remove SAM*.*
