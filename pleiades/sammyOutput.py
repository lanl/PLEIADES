...
class LptFile:
                ...
                for line in fid:
                    ...
                    if line.startswith(pattern["start_text"]):
                        ...
                        try:
                            ...
                            warnings.warn(
                                f"pattern key: {pattern_key} has returned wrong parsing result"
                            )

        return stats

    def register_new_stats(
        self, keyname: str, start_text: str, skipped_rows: str, line_format: str
    ) -> None:
        """
        ...
        """
        self.LPT_SEARCH_PATTERNS[keyname] = dict(
            start_text=start_text, skipped_rows=skipped_rows, line_format=line_format
        )
    ...
    def register_normalization_stats(self, register_vary: bool = False) -> None:
        ...
        """
        self.register_new_stats(
            "normalization", "   NORMALIZATION", 1, "float(line[1:12])"
        )
        self.register_new_stats(
            "constant_bg", "   NORMALIZATION", 1, "float(line[18:29])"
        )
        self.register_new_stats(
            "one_over_v_bg", "   NORMALIZATION", 1, "float(line[34:46])"
        )
        self.register_new_stats(
            "sqrt_energy_bg", "   NORMALIZATION", 1, "float(line[51:63])"
        )
        self.register_new_stats(
            "exponential_bg", "    BCKG*EXP(.)", 1, "float(line[1:12])"
        )
        self.register_new_stats(
            "exp_decay_bg", "    BCKG*EXP(.)", 1, "float(line[18:29])"
        )
        ...
        if register_vary:
            self.register_new_stats(
                ...
                "   NORMALIZATION",
                1,
                "1 if line[13:17].strip() else 0",
            )
            self.register_new_stats(
                "vary_constant_bg",
                "   NORMALIZATION",
                1,
                "1 if line[30:34].strip() else 0",
            )
            self.register_new_stats(
                "vary_one_over_v_bg",
                "   NORMALIZATION",
                1,
                "1 if line[47:51].strip() else 0",
            )
            self.register_new_stats(
                "vary_sqrt_energy_bg",
                "   NORMALIZATION",
                1,
                "1 if line[64:68].strip() else 0",
            )
            self.register_new_stats(
                "vary_exponential_bg",
                "    BCKG*EXP(.)",
    ...
    def register_broadening_stats(self, register_vary: bool = False) -> None:
        ...
        """
        self.register_new_stats(
            "temperature", "  TEMPERATURE      THICKNESS", 1, "float(line[1:12])"
        )
        self.register_new_stats(
            "thickness", "  TEMPERATURE      THICKNESS", 1, "float(line[18:29])"
        )
        self.register_new_stats(
            "flight_path_spread",
            "    DELTA-L         DELTA-T-GAUS",
            1,
            "float(line[1:12])",
        )
        self.register_new_stats(
            "deltag_fwhm", "    DELTA-L         DELTA-T-GAUS", 1, "float(line[18:29])"
        )
        self.register_new_stats(
            "deltae_us", "    DELTA-L         DELTA-T-GAUS", 1, "float(line[35:46])"
        )
        ...
        if register_vary:
            self.register_new_stats(
                ...
                1,
                "1 if line[12:17].strip() else 0",
            )
            self.register_new_stats(
                "vary_thickness",
                "  TEMPERATURE      THICKNESS",
                1,
                "1 if line[29:34].strip() else 0",
            )
            self.register_new_stats(
    ...
    def register_misc_stats(self, register_vary: bool = False) -> None:
        ...
        if register_vary:
            self.register_new_stats(
                "vary_t0", "    t-zero", 0, "1 if line[24:29].strip() else 0"
            )
            self.register_new_stats(
                "vary_L0", "    t-zero", 0, "1 if line[53:58].strip() else 0"
            )
            self.register_new_stats(
                "vary_delta_L1", "    Delta-L", 0, "1 if line[24:29].strip() else 0"
            )
            self.register_new_stats(
                "vary_delta_L0", "    Delta-L", 0, "1 if line[61:66].strip() else 0"
            )
            self.register_new_stats(
                "vary_DE", "    Delta-E=", 0, "1 if line[24:29].strip() else 0"
            )
            self.register_new_stats(
                "vary_D0", "    Delta-E=", 0, "1 if line[63:68].strip() else 0"
            )
            self.register_new_stats(
                "vary_DlnE", "    Delta-E=", 1, "1 if line[24:29].strip() else 0"
            )
    ...
    def register_abundances_stats(self, isotopes: list = []) -> None:
        ...
        if isotopes:
            for num, isotope in enumerate(isotopes):
                self.register_new_stats(
                    f"{isotope}", " Nuclide    Abundance", num + 1, "float(line[12:18])"
                )
        else:
            for num in range(5):
                self.register_new_stats(
    ...
    def param_table(
        self, only_vary: bool = True, dataframe: bool = True
        ...
        if uncertainties:
            final_params = {
                vary_key: (params[key], uncertainties[key], "1")
                for vary_key, key in zip(initial_params, params)
            }
        else:
            final_params = {
                vary_key: (params[key], "", "1")
                for vary_key, key in zip(initial_params, params)
            }
        if only_vary:
            ...
            initial_params = {
                key: (
                    all_initial_params[key]
                    if key not in final_params
                    else final_params[key]
                )
                for key in all_initial_params
            }
        ...
        if dataframe:
            return pandas.DataFrame(
                self.final_params, index=["param", "uncertainty", "vary"]
            ).T
        else:
...
class ParamsConfig:
    ...
    def get_vary_params(self, only_vary: bool = True) -> dict:
        ...
        if only_vary:
            vary_params = {
                key: value
                for key, value in self.flatten_params.items()
                if key.startswith("vary_") and value == "1"
            }
        else:
            vary_params = {
                key: value
                for key, value in self.flatten_params.items()
                if key.startswith("vary_")
            }
        ...
        for isotope in eval(self.flatten_params["isotopes"]):
            if only_vary:
                if self.flatten_params[f"vary_{isotope}"] == "1":
                    sorted_params[isotope] = (
                        self.flatten_params[isotope],
                        0.0,
                        self.flatten_params[f"vary_{isotope}"],
                    )
            else:
                sorted_params[isotope] = (
                    self.flatten_params[isotope],
        ...
        for key in params_order:
            if f"vary_{key}" in vary_params:
                ...
                sorted_params[key] = (
                    self.flatten_params[key],
                    0.0,
                    vary_params[f"vary_{key}"],
                )
