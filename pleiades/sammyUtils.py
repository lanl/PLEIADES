    ...
    def _load_from_config(self, config):
        ...
        for section in config.sections():
            ...
            if section == "isotopes":
                ...
                self.params["isotopes"]["abundances"] = [
                    float(x) for x in config.get("isotopes", "abundances").split(",")
                ]
                self.params["isotopes"]["vary_abundances"] = [
                    self._convert_value(x)
                    for x in config.get("isotopes", "vary_abundances").split(",")
                ]
            ...
            # If the section is 'filenames'
                ...
                self.params["run_with_endf"] = self._convert_value(
                    config.get("main", "run_with_endf")
                )
                self.params["fit_energy_min"] = float(
                    config.get("main", "fit_energy_min")
                )
                self.params["fit_energy_max"] = float(
                    config.get("main", "fit_energy_max")
                )
                self.params["flight_path_length"] = float(
                    config.get("main", "flight_path_length")
                )
                self.params["fudge_factor"] = float(config.get("main", "fudge_factor"))

            elif section == "resonances":
                self.params["resonances"]["resonance_energy_min"] = float(
                    config.get("resonances", "resonance_energy_min")
                )
                self.params["resonances"]["resonance_energy_max"] = float(
                    config.get("resonances", "resonance_energy_max")
                )
    ...
    def _check_or_create_directories(self):
        ...
        # otherwise create the subdirectories within the working directory
            ...
            fit_results_dir = os.path.join(
                working_dir, self.params["directories"]["fit_results_dir"]
            )
            archive_dir = os.path.join(
                working_dir, self.params["directories"]["archive_dir"]
            )
            endf_dir = os.path.join(archive_dir, self.params["directories"]["endf_dir"])
            sammy_fit_dir = os.path.join(
                archive_dir, self.params["directories"]["sammy_fit_dir"]
            )
...
def create_parFile_from_endf(
    ...
    for isotope in isotopes:
        ...
        if archive:
            ...
            if verbose_level > 0:
                print(f"SAMMY input file created at {sammy_input_file}")
...
def configure_sammy_run(config: SammyFitConfig, verbose_level: int = 0):
    ...
    if verbose_level > 0:
        print(
            f"Creating SAMMY parFile files for isotopes: {isotopes} with abundances: {abundances}"
        )
    ...
    broadening_args = {
        ...
        "vary_match_radius": int(
            config.params["broadening"]["vary_match_radius"] == True
        ),
        ...
        "vary_flight_path_spread": int(
            config.params["broadening"]["vary_flight_path_spread"] == True
        ),
    ...
    normalization_args = {
        ...
        "vary_normalization": int(
            config.params["normalization"]["vary_normalization"] == True
        ),
        "vary_constant_bg": int(
            config.params["normalization"]["vary_constant_bg"] == True
        ),
        "vary_one_over_v_bg": int(
            config.params["normalization"]["vary_one_over_v_bg"] == True
        ),
        "vary_sqrt_energy_bg": int(
            config.params["normalization"]["vary_sqrt_energy_bg"] == True
        ),
        "vary_exponential_bg": int(
            config.params["normalization"]["vary_exponential_bg"] == True
        ),
        "vary_exp_decay_bg": int(
            config.params["normalization"]["vary_exp_decay_bg"] == True
        ),
    }
    ...
    output_par_file = (
        Path(archive_dir) / Path(sammy_fit_dir) / Path("params").with_suffix(".par")
    )
    if verbose_level > 0:
        print(f"Writing output parFile: {output_par_file}")
    outputParFile.write(output_par_file)

    # Now create a SAMMY input file
    if verbose_level > 0:
        print(
            f"Creating SAMMY inpFile files for isotopes: {isotopes} with abundances: {abundances}"
        )
    ...
    if verbose_level > 0:
        print(f"Created compound input file: {output_inp_file}")
    ...
    if verbose_level > 0:
        print(f"Symlinking data file: {data_file} into {sammy_fit_dir}")
...
def run_sammy(config: SammyFitConfig, verbose_level: int = 0):
    ...
    with open(output_file, "r") as file:
        ...
        if "Normal finish to SAMMY" in data:
            sammy_success = True
            if verbose_level > 0:
                print("SAMMY executed successfully.")
        else:
            sammy_success = False
...
def run_sammy_fit(
    ...
    for isotope, abundance in abundances.items():
        isotopes.append(
            sammyParFile.ParFile(
                f".archive/{isotope}/results/{isotope}.par",
                weight=abundance,
                emin=res_emin,
                emax=res_emax,
            ).read()
    ...
    if (
        vary_resonance_energies is not None
        or vary_gamma_widths is not None
        or vary_neutron_widths is not None
    ):
        compound.update.vary_resonances_in_energy_range(
            vary_energies=vary_resonance_energies,
    ...
    if not atomic_weight:
        atomic_weight = np.sum(
            [
                int(re.search(r"\d+", isotope).group()) * abundance
                for isotope, abundance in abundances.items()
            ]
        )
    ...
    inp.data["Card5"]["dist"] = (
        params["flight_path_length"]
        if "flight_path_length" in params and params["flight_path_length"]
        else flight_path_length
    )
    inp.data["Card5"]["temp"] = 296.0

    # update commands. We need to preform the REICH_MOORE and SOLVE_BAYES for this case

    default_commands = set(
        [
            "CHI_SQUARED",
            "TWENTY",
            "SOLVE_BAYES",
    ...
    # write the inp file
    inp.process(auto_update=False).write(
        "archive" / archivename / archivename.with_suffix(".inp")
    )

    # run sammy, it saves the results inside the "./archive/W/results" directory
    sammyRunner.run(
        archivename=archivename.stem,
    ...
    output = sammyOutput.LptFile(
        "archive"
        / Path(archivename.stem)
        / "results"
        / Path(archivename.stem).with_suffix(".lpt")
    )
...
def print_stats_dict(stats: dict) -> str:
    ...
    isotopes = ",\t".join(
        [f"{key}:\t{stats[key]}" for key in stats if nucData.get_mass_from_ame(key)]
    )
