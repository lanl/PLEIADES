...
def create_parFile_from_endf(config: SammyFitConfig, archive: bool = True, verbose_level: int = 0) -> None:
    ...
    if not sammyRunner.check_sammy_environment(config, verbose_level):
        ...
        if verbose_level > 0: print(f"{print_header_good} SAMMY executable found in the path")
    
    # First grab the isotopes from the configuration class
    isotopes = config.params['isotopes']['names']
    if verbose_level > 1: print(f"{print_header_check} Isotopes: {isotopes}")
    ...
    if not os.path.exists(endf_dir_path):
        ...
        if verbose_level > 0: print(f"{print_header_check} ENDF directory at {endf_dir_path} already exists")  
    ...
    for isotope in isotopes: 
        ...
        if not os.path.exists(isotope_dir_path):
            ...
            if verbose_level > 0: print(f"{print_header_check} Isotope directory created at {isotope_dir_path}")
        else:
            if verbose_level > 0: print(f"{print_header_check} Isotope directory at {isotope_dir_path} already exists")
            ...
            if os.path.exists(par_file):
                ...
                if verbose_level > 0: print(f"{print_header_check} A SAMMY parFile does not exist at {par_file}")
        
        # Print a header if verbose_level > 0
        if verbose_level > 0: print(f"{print_header_check} Now Creating SAMMY input.inp and params.par for isotope: {isotope}")
        ...
        if verbose_level > 0: print(f"{print_header_check} ENDF file: {endf_par_file}")
...
def configure_sammy_run(config: SammyFitConfig, verbose_level: int = 0):
    ...
    if config.params['use_endf_par_file'] == True:
